...
def _text_looks_like_sj_may_refrigeration(text: str) -> bool:
    normalized = (text or "").lower()
    return (
        "mayrefrigeration" in normalized
        or "s&j may" in normalized
        or "3965960" in normalized
    )
...
def _text_looks_like_dr_organic(text: str) -> bool:
    normalized = (text or "").lower()
    return (
        "drorganic.co.uk" in normalized
        or "dr organic ltd" in normalized
        or "934626805" in normalized
    )
...
async def sage_test_img2pdf(request: Request) -> Dict[str, Any]:
    ...
    try:
        import img2pdf  # type: ignore[import]
        version = getattr(img2pdf, "__version__", "unknown")
        ...
        pdf_bytes = img2pdf.convert(minimal_jpeg)
        return {
            "status": "ok",
            "img2pdf_version": version,
            "pdf_size_bytes": len(pdf_bytes),
            "pdf_starts_with": pdf_bytes[:5].decode("latin-1"),
        }
    except Exception as exc:
...
def _detect_and_parse(
    ...
    elif _text_looks_like_sj_may_refrigeration(text):
        return [parse_sj_may_refrigeration(text)]
