...
def _extract_invoice_number(text: str) -> Optional[str]:
    match = re.search(r"\b(INV-\d+)\b", text or "")
    return match.group(1) if match else None


def _extract_issue_date(text: str) -> Optional[str]:
    """Issue date sits directly before the invoice number, e.g. '29 Aug 2026 INV-1085'."""
    match = re.search(r"(\d{1,2}\s+\w+\s+\d{4})\s+INV-\d+", text or "")
    return match.group(1) if match else None


def _extract_due_date(text: str) -> Optional[str]:
    """Due date sits directly after the amount due, e.g. '£149.83 28 Sept 2026'."""
    match = re.search(r"£[\d,]+\.\d{2}\s+(\d{1,2}\s+\w+\s+\d{4})", text or "")
    return match.group(1) if match else None
...
def _extract_totals(text: str) -> tuple[Optional[float], Optional[float], Optional[float]]:
    ...
    m = re.search(r"Subtotal\s+([\d,]+\.\d{2})", text or "")
    if m:
        net = parse_money(m.group(1))

    m = re.search(r"Total VAT\s+[\d.]+%\s+([\d,]+\.\d{2})", text or "", re.IGNORECASE)
    if m:
        vat = parse_money(m.group(1))

    m = re.search(r"^Total\s+([\d,]+\.\d{2})\s*$", text or "", re.MULTILINE)
    if m:
...
def parse_breccles(text: str) -> InvoiceData:
    warnings: list[str] = []
    ...
    invoice_date = parse_date(_extract_issue_date(text or ""))
    if not invoice_date:
    ...
    return InvoiceData(
        ...
        contact_id="bde4b37c78874a18a734d855f0ac1438",
