...
def _extract_invoice_number(text: str) -> Optional[str]:
    match = re.search(r"\b(SIN\d+)\b", text or "")
    return match.group(1) if match else None


def _extract_invoice_date(text: str) -> Optional[str]:
    # Tax Point label followed by date on same or next token
    match = re.search(
        r"Tax\s+Point\s+.*?(\d{1,2}/\d{1,2}/\d{4})",
        text or "",
...
def _extract_totals(text: str) -> tuple[Optional[float], Optional[float], Optional[float]]:
    ...
    vat = None
    total = None
    m = re.search(r"Net\s+Total\s+[£$]?\s*([\d,]+\.\d{2})", text, flags=re.IGNORECASE)
    if m:
        net = parse_money(m.group(1))
    m = re.search(r"VAT\s+Content\s+[£$]?\s*([\d,]+\.\d{2})", text, flags=re.IGNORECASE)
    if m:
        vat = parse_money(m.group(1))
    # "Total" but not "Net Total" or "VAT Content"
    m = re.search(r"(?<!Net\s)(?<!VAT\s)(?<!\w)Total\s+[£$]?\s*([\d,]+\.\d{2})", text, flags=re.IGNORECASE)
    if m:
        total = parse_money(m.group(1))
    return net, vat, total


def parse_absolute_aromas(text: str) -> InvoiceData:
    warnings: list[str] = []
