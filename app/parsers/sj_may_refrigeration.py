...
def _extract_invoice_number(text: str) -> Optional[str]:
    match = re.search(r"Invoice No\s+(\d+)", text or "", re.IGNORECASE)
    return match.group(1) if match else None


def _extract_invoice_date(text: str) -> Optional[str]:
    match = re.search(r"Invoice Date\s+(\d{1,2}/\d{2}/\d{4})", text or "", re.IGNORECASE)
    return match.group(1) if match else None
...
def _extract_totals(text: str) -> tuple[Optional[float], Optional[float], Optional[float]]:
    ...
    m = re.search(r"Total Net Amount\s+£\s*([\d,]+\.\d{2})", text or "", re.IGNORECASE)
    if m:
        net = parse_money(m.group(1))

    m = re.search(r"Total Tax Amount\s+£\s*([\d,]+\.\d{2})", text or "", re.IGNORECASE)
    if m:
        vat = parse_money(m.group(1))

    m = re.search(r"Invoice Total\s+£\s*([\d,]+\.\d{2})", text or "", re.IGNORECASE)
    if m:
...
def parse_sj_may_refrigeration(text: str) -> InvoiceData:
    warnings: list[str] = []
    ...
    return InvoiceData(
        supplier="S&J May Refrigeration",
        supplier_reference=invoice_number,
        invoice_date=invoice_date,
        due_date=due_date,
        deliver_to_postcode=postcode,
        ledger_account=ledger_account,
        contact_id="df5f42b738b14b359077b445eeaae1c7",
        vat_net=round(net, 2),
        nonvat_net=0.0,
